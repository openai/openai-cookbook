...
- Keep task scope narrow and aligned to the issue statement; avoid broad repository rescans unless explicitly required.
- For PR review-comment tasks, touch only files directly referenced by the review and stop after producing the requested artifact.
- Prefer targeted tests over suite-wide runs, then stop when the requested validation is complete.
- Keep prompts and progress updates concise to reduce cloud-agent token usage and avoid quota-based workflow failures.
- Store evolving operational learnings in dedicated docs/checkpoints near the affected subsystem rather than expanding this global agent context.
