...
The following excerpts explain [agent.py](agent.py) and [warehouse.py](warehouse.py).
...
[agent.py](agent.py) defines a schema for each tool, then enables tool search and
...
application-owned: [memory.py](memory.py) saves corrections in `memories.json`.
