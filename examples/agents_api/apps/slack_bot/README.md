...
The snippets below show the main steps in [main.py](main.py), [agent.py](agent.py),
and [connections.py](connections.py). Run the application above for the complete
...
Slack tools run in your application with the bot token. [tools.py](tools.py)
...
container when the conversation expires. [SlackBot.close()](agent.py) does this
