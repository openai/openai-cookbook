...
async def main() -> None:
    ...
    setup = await sandbox.process.exec(
        {
            "command": "apk add --no-cache python3 py3-pip && python3 -m venv /opt/controller && /opt/controller/bin/pip install uv && mkdir -p /app/webhook_managed",
    ...
    await sandbox.fs.write(
        "/app/webhook_managed/handler.py", ROOT.joinpath("handler.py").read_text()
    )
    await sandbox.fs.write(
        "/app/sandbox.py", ROOT.parent.joinpath("sandbox.py").read_text()
    )
    install = await sandbox.process.exec(
        {
            "command": "/opt/controller/bin/uv pip install --python /opt/controller/bin/python -r /app/webhook_managed/handler.py",
            "wait_for_completion": True,
            "timeout": 180,
        }
    )
    ...
    await sandbox.process.exec(
        {
            ...
            "working_dir": "/app/webhook_managed",
            "env": values,
