...
async def main() -> None:
    ...
    async with AsyncDaytona() as daytona:
        ...
        await sandbox.fs.create_folder("/app/webhook_managed", "755")
        await sandbox.fs.upload_file(
            ROOT.joinpath("handler.py").read_bytes(), "/app/webhook_managed/handler.py"
        )
        await sandbox.fs.upload_file(
            ROOT.parent.joinpath("sandbox.py").read_bytes(), "/app/sandbox.py"
        )
        install = await sandbox.process.exec(
            "python -m pip install uv && uv pip install --system -r /app/webhook_managed/handler.py",
            timeout=180,
        )
        ...
        await sandbox.process.execute_session_command(
            "webhook-controller",
            SessionExecuteRequest(
                command="cd /app/webhook_managed && python -m uvicorn handler:app --host 0.0.0.0 --port 8000 >> /app/controller.log 2>&1",
                run_async=True,
            ),
        )
