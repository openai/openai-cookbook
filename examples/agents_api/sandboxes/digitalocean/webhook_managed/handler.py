...
MANIFEST = Path(__file__).resolve().parents[1] / "environment.yaml"
...
async def reconcile(openai_session_id: str) -> None:
    ...
    async with do:
        ...
        if existing is not None:
            ...
            if existing.get("status") == "SESSION_STATUS_PAUSED":
                try:
                    ...
                    # PyDo 0.41.0 can fail to parse a successful empty response.
                    # Confirm the resume before acknowledging this delivery.
                    async with asyncio.timeout(10):
                        for attempt in range(5):
                            current = await do.agents.sessions.get(
                                existing["session_id"]
                            )
                            if current["session"]["status"] == "SESSION_STATUS_READY":
                                break
                            if attempt == 4:
                                raise
        ...
        response = await do.agents.create_session(
            ...
            body={
                "manifest": MANIFEST.read_text().replace("{name}", name),
                "variables": {
