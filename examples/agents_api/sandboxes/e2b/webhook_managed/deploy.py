...
async def main() -> None:
    ...
    await sandbox.commands.run("mkdir -p /app/webhook_managed", user="root")
    await sandbox.files.write(
        "/app/webhook_managed/handler.py",
        ROOT.joinpath("handler.py").read_text(),
        user="root",
    )
    await sandbox.files.write(
        "/app/sandbox.py", ROOT.parent.joinpath("sandbox.py").read_text(), user="root"
    )
    await sandbox.commands.run(
        "python -m venv /opt/controller && /opt/controller/bin/pip install uv && /opt/controller/bin/uv pip install --python /opt/controller/bin/python -r /app/webhook_managed/handler.py",
        user="root",
        timeout=180,
    )
    await sandbox.commands.run(
