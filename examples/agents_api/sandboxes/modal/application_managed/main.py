...
# Keep direct `uv run main.py` working from this directory.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from modal_executor import WORKSPACE, exec_server_command, executor_image  # noqa: E402
...
async def start_modal_sandbox(
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run: uv run examples/agents_api/sandboxes/modal/application_managed/main.py"
        ) from exc

    app = await modal.App.lookup.aio(DEFAULT_MODAL_APP_NAME, create_if_missing=True)
    image = (
        executor_image(python_version="3.14")
        .apt_install(
            "ca-certificates",
            "curl",
            "file",
