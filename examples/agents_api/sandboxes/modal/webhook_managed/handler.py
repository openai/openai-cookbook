...
"""Deploy with: uv run examples/agents_api/sandboxes/modal/webhook_managed/handler.py."""
...
# The controller image packages this provider-local module for remote imports.
sys.path.insert(0, str(Path(__file__).resolve().parents[1]))
from modal_executor import WORKSPACE, exec_server_command  # noqa: E402
from modal_executor import executor_image as base_image  # noqa: E402

APP_NAME = "agents-api-webhook-modal"
app = modal.App(APP_NAME)
executor_image = base_image()
controller_image = executor_image.pip_install(
    "fastapi[standard]", "openai>=3.13.0"
...
async def reconcile(session_id: str) -> None:
    ...
    sandbox = await modal.Sandbox.create.aio(
        *exec_server_command(environment.id, environment.remote_url),
        app=app,
        name=session_id,
