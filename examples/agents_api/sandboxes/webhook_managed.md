...
- **Application:** each provider's `webhook_managed/client.py` creates sessions,
  sends input, and streams results. It imports no provider SDK.
...
| --- | --- | --- |
| Modal | [handler.py](modal/webhook_managed/handler.py) | [Deploy](modal/webhook_managed/README.md) |
| Vercel | [api/webhook.ts](vercel/webhook_managed/api/webhook.ts) and [api/provision.ts](vercel/webhook_managed/api/provision.ts) | [Deploy](vercel/webhook_managed/README.md) |
| Cloudflare | [src/index.ts](cloudflare/webhook_managed/src/index.ts) | [Deploy](cloudflare/webhook_managed/README.md) |
| Blaxel | [handler.py](blaxel/webhook_managed/handler.py) | [Deploy](blaxel/webhook_managed/README.md) |
| Daytona | [handler.py](daytona/webhook_managed/handler.py) | [Deploy](daytona/webhook_managed/README.md) |
| E2B | [handler.py](e2b/webhook_managed/handler.py) | [Deploy](e2b/webhook_managed/README.md) |
| Runloop | [handler.py](runloop/webhook_managed/handler.py) | [Deploy](runloop/webhook_managed/README.md) |
| DigitalOcean | [handler.py](digitalocean/webhook_managed/handler.py) | [Deploy](digitalocean/webhook_managed/README.md) |

Complete the setup below, deploy one provider's handler, then run its
client. For direct provisioning from your application, use
[application-managed sandboxes](README.md).

## Set up once

1. Install [uv](https://docs.astral.sh/uv/getting-started/installation/) and choose
   your provider folder. For example, from the Cookbook repository root:

   ```bash
   cd examples/agents_api/sandboxes/modal/webhook_managed
   ```

   Run the client commands below from that folder. Each Python script declares
   its dependencies inline; `uv run` installs them automatically.
2. Set your application's `OPENAI_API_KEY`, then create an agent:

   ```bash
   uv run client.py --create-agent sandbox-demo
   ```
...
```bash
uv run client.py --session-id "$SESSION_ID" --delete
```
