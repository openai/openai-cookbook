...
def num_tokens_consumed_from_request(
    ...
    if api_endpoint == "completions":
        ...
        elif request_json.get("messages"):
            messages = request_json["messages"]
            completion_tokens = n * max_tokens
            if isinstance(messages, list):
                messages_tokens = 0
                for message in messages:
                    messages_tokens += 4  # every message follows <im_start>{role/name}\n{content}<im_end>\n
                ...
                messages_tokens += 2  # every reply is primed with <im_start>assistant
                num_tokens = messages_tokens + completion_tokens
                return num_tokens
            else:
                raise TypeError(
                    'Expecting list of dicts for "messages" field in completion request'
                )
