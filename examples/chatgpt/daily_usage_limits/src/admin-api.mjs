  ...
  return {
    ...
    async listMembers() {
      ...
      for (let page = 0; page < maxPages; page += 1) {
        ...
        if (result.object !== 'list' || !Array.isArray(result.data) || typeof result.has_more !== 'boolean') {
          throw fail('MEMBER_PAGE_INVALID');
        }
        for (const row of result.data) {
          if (row.object !== 'workspace.user' || !validId(row.id) || ids.has(row.id)) {
            throw fail('MEMBER_PAGE_INCONSISTENT');
          }
          ids.add(row.id);
          if (ids.size > maxRows) throw fail('MEMBER_ROW_BOUND_EXCEEDED');
        }
        if (!result.has_more) return [...ids].sort();
        if (!result.data.length || result.last_id !== result.data.at(-1).id || result.last_id === after) {
          throw fail('MEMBER_CURSOR_INVALID');
        }
    ...
    async readHistory(userId, { start, end, unit }) {
      ...
      for (let page = 0; page < maxPages; page += 1) {
        const query = new URLSearchParams({ start_time: String(startMs / 1000),
          end_time: String(endMs / 1000), limit: '1000', ...(cursor ? { page: cursor } : {}) });
        const result = await request(`/analytics/workspaces/${encodeURIComponent(workspaceId)}/usage?${query}`);
        if (result.object !== 'page' || !Array.isArray(result.data) || typeof result.has_more !== 'boolean') {
          throw fail('HISTORY_PAGE_INVALID');
        }
        rows += result.data.length;
        if (rows > maxRows) throw fail('HISTORY_ROW_BOUND_EXCEEDED');
        for (const row of result.data) {
          if (row.object !== 'workspace.usage.result' || !validId(row.user_id)) throw fail('HISTORY_SCOPE_INVALID');
        ...
        if (!result.has_more) {
          ...
          return { workspaceId, userId, unit, start: new Date(startMs).toISOString(),
            end: new Date(endMs).toISOString(), observedAt: now(), days, semantics: 'observed' };
        }
