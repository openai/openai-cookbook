...
def top_k_tokens(text: str, k: int) -> List[Tuple[str, int]]:
    ...
    # If requesting a large fraction, sort all; otherwise use bounded selection
    if m >= u or (m * 10 >= 3 * u):
        return sorted(cnt.items(), key=key)[:m]
...
# Complexity: counting O(N tokens); selection O(U log min(k, 0.3U)) via heap/partial sort; extra space O(U + min(k, 0.3U))