...
def top_k_tokens(text: str, k: int) -> List[Tuple[str, int]]:
    ...
    # Exact selection with bounded memory: force heap path by passing an iterator (no __len__)
    return heapq.nsmallest(k, iter(cnt.items()), key=key)
...
# Complexity: counting O(T) over tokens; selection O(U log k); extra space O(U + k).