...
commands. Unless a command block says otherwise, run it from the repository
root. Commands that target a component use `npm --prefix <component>` or
...
Then run the bounded, read-only AWS verification from the repository root:
