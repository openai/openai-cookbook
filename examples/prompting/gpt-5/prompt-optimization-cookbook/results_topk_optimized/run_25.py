...
# Use provided globals if present; otherwise, run a small demo only when executed as a script
try:
...
# Complexity: counting O(N tokens), selection O(U log k) via heap; extra space O(U + k)