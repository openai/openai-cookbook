...
# Set top_k from provided globals if present; otherwise optional demo under __main__
try:
...
# Complexity: counting O(N tokens); selection O(U log k) with heapq.nsmallest or O(U log U) when k >= U; extra space O(U + k).