...
1. [Creating a SQL table](#create-a-vector-table) that can store vector data
1. [Generating OpenAI embeddings](#generate-openai-embeddings) using OpenAI's JavaScript client
1. [Storing the embeddings](#store-embeddings-in-database) in your SQL table using the Supabase JavaScript client
1. [Performing semantic search](#semantic-search) over the embeddings using a Postgres function and the Supabase JavaScript client
...
This index uses the [HNSW](https://supabase.com/docs/guides/ai/vector-indexes/hnsw-indexes) algorithm to index vectors stored in the `embedding` column, and specifically when using the cosine distance operator (`<=>`). We'll explain more about this operator later when we implement our match function.
...
- Filters the documents to only include those who's embeddings match within the above `match_threshold`. Since the `<=>` operator performs cosine distance (not cosine similarity), we subtract the similarity threshold from 1 to convert it into distance before comparing. This means a `match_threshold` of 1 is most similar, and -1 is most dissimilar.
- Orders the documents by cosine distance (`<=>`) ascending. This allows us to retrieve documents that match closest first.
