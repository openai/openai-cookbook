...
1. **Full refresh** (billing + HubSpot changed):
   ...
   ```
   Runs: build → populate_deal_associations → populate_accountant_deals → MRR + ICP dashboards → Building Blocks budget dashboard.
...
3. **Building Blocks only** (Google Sheets updated):
   ```bash
   python tools/scripts/refresh_budget_dashboard.py --output docs/budget_dashboard.html
   ```

4. **After fixing deal associations** (e.g. redundant type 8 removal, Group 4):
   ```bash
...
See [DASHBOARD_REFRESH_GUIDE.md](./DASHBOARD_REFRESH_GUIDE.md) for full details.
