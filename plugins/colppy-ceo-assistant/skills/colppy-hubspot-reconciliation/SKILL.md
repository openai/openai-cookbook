...
| **3. Wrong Stage** | **COLPPY_NOT_ACTIVE:** Colppy activa ≠ 0, HubSpot closed-won. **WRONG_STAGE:** Deal exists (e.g. Cerrado Churn) but not closed-won. **NO_HUBSPOT_DEAL:** No deal in HubSpot. |
...
- `id_empresa`, `reason` (group3 only)
- `colppy_id_plan`, `hubspot_id_plan`
...
3. If month not in snapshot, tell the user to run locally and republish:
   ```bash
   ...
   ```
4. **Present in chatbot:** Summary table + all 4 groups + **full deal list with clickable HubSpot links**. Do not only reference .md files — display results in chat.
