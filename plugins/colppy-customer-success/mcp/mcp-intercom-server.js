...
/**
 * Intercom MCP Server (Colppy CEO Assistant plugin)
 *
...
// Load .env from workspace root or plugin root (INTERCOM_ACCESS_TOKEN from MCP config takes precedence)
...
class IntercomMCPServer {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          case 'analyze_onboarding_first_invoice': return await this.analyzeOnboardingFirstInvoice(args);
          default:
  ...
  async countConversations(args) {
    const { from_date, to_date = new Date().toISOString().split('T')[0], state, team_assignee_id } = args;
    try {
      const searchQuery = buildSearchQuery({ from_date, to_date, state, team_assignee_id });
      let total = 0;
      ...
      return this._json({ success: true, date_range: { from: from_date || null, to: to_date }, state: state || null, team_assignee_id: team_assignee_id || null, total_count: total });
    } catch (error) {
  ...
  async exportConversations(args) {
    const { from_date, to_date = new Date().toISOString().split('T')[0], team_assignee_id, output_format = 'csv', limit, include_message_content = true } = args;
    try {
      if (!process.env.INTERCOM_ACCESS_TOKEN) throw new Error('INTERCOM_ACCESS_TOKEN not found');
      const conversationIds = await fetchConversationIds(this.intercomApi, from_date, to_date, limit, null, team_assignee_id);
      if (conversationIds.length === 0) {
        return this._json({ success: true, message: 'No conversations found', date_range: { from: from_date, to: to_date }, team_assignee_id: team_assignee_id || null, total_conversations: 0, data: [] });
      }
      ...
      return this._json({
        ...
        team_assignee_id: team_assignee_id || null,
  ...
  async getConversationStats(args) {
    const { from_date, to_date = new Date().toISOString().split('T')[0], team_assignee_id } = args;
    try {
      const ids = await fetchConversationIds(this.intercomApi, from_date, to_date, 100, null, team_assignee_id);
  ...
  async searchConversations(args) {
    const { query, from_date, to_date = new Date().toISOString().split('T')[0], state, team_assignee_id, limit = 50 } = args;
    try {
      const searchQuery = buildSearchQuery({ from_date, to_date, state, team_assignee_id, per_page: Math.min(limit, 150) });
      const response = await retryableRequest(() => this.intercomApi.post('/conversations/search', searchQuery));
      ...
      return this._json({
        success: true, search_criteria: { query, from_date, to_date, state, team_assignee_id: team_assignee_id || null, limit },
  ...
  async scanCustomerFeedback(args) {
    ...
    try {
      ...
      return this._json({
        ...
        search_criteria: { keywords, tags: tagFilter || [], from_date, to_date, state: state || null, team_assignee_id: team_assignee_id || null },
  ...
  async scanFullText(args) {
    ...
    try {
      ...
      const probeIds = await fetchConversationIds(this.intercomApi, from_date, to_date, LARGE_SCAN_THRESHOLD + 1, state, team_assignee_id);
      ...
      if (totalInRange > LARGE_SCAN_THRESHOLD && !confirm_large_scan) {
        return this._json({
          ...
          search_criteria: { topic, from_date, to_date, state: state || null, team_assignee_id: team_assignee_id || null },
        });
      }

      const scanLimit = confirm_large_scan ? null : Math.min(max_scan, totalInRange);
      const conversationIds = confirm_large_scan
        ? await fetchConversationIds(this.intercomApi, from_date, to_date, null, state, team_assignee_id)
      ...
      if (save_cache && cacheConversations && cacheConversations.length > 0) {
        cachePathResolved = save_cache.startsWith('/') ? save_cache : join(__dirname, '..', save_cache);
  ...
  /**
   * Run onboarding analysis (Python script).
   * Filters by user_type: accountant (es_contador=true), smb (es_contador=false), or all.
   * Supports LLM classification with configurable topic.
   */
  async analyzeOnboardingFirstInvoice(args) {
    const { cache_path, user_type = 'accountant', use_llm = false, topic } = args;
    const cacheResolved = cache_path.startsWith('/')
      ? cache_path
      : join(REPO_ROOT, cache_path);
    if (!fs.existsSync(cacheResolved)) {
      throw new McpError(ErrorCode.InvalidParams, `Cache file not found: ${cacheResolved}. Run export_cache_for_local_scan.mjs --team 2334166 first.`);
    }
    const scriptPath = join(REPO_ROOT, 'tools', 'scripts', 'intercom', 'analyze_onboarding.py');
    if (!fs.existsSync(scriptPath)) {
      throw new McpError(ErrorCode.InternalError, `Analysis script not found: ${scriptPath}`);
    }
    return new Promise((resolve, reject) => {
      const pythonCmd = process.platform === 'win32' ? 'python' : 'python3';
      const procArgs = [scriptPath, '--cache', cacheResolved, '--user-type', user_type, '--json'];
      if (use_llm) procArgs.push('--llm');
      if (topic) procArgs.push('--topic', topic);
      ...
      proc.on('close', (code) => {
        if (code !== 0) {
          reject(new McpError(ErrorCode.InternalError, `Analysis failed (exit ${code}): ${stderr || stdout}`));
          return;
        }
        try {
          const result = JSON.parse(stdout.trim());
          resolve(this._json(result));
        } catch (e) {
          resolve(this._json({ success: true, report: stdout, parse_error: e.message }));
        }
      });
      proc.on('error', (err) => reject(new McpError(ErrorCode.InternalError, `Failed to run analysis: ${err.message}`)));
    });
  }
