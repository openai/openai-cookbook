...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (args[i] === '--review' && args[i + 1]) opts.review = args[++i];
    else if (args[i] === '--apply-review' && args[i + 1]) opts.applyReview = args[++i];
...
${config.definition}
  ...
  // Inject few-shot examples if provided in the topic config
  if (config.few_shot_examples?.length) {
    prompt += `\n\n<examples>`;
    for (const ex of config.few_shot_examples) {
      prompt += `\n\nExample conversation:\n${ex.conversation_snippet}\n\nCorrect classification:\n${JSON.stringify(ex.classification, null, 2)}`;
    }
    prompt += `\n</examples>`;
  }
...
async function main() {
  ...
  // ── Review mode: write review file for human verification ───────────
  if (opts.review) {
    const reviewPath = resolvePath(opts.review) || opts.review;
    const reviewData = {
      topic: topicName,
      ...
      total_classified: classified.length,
      matches: tp.length,
      reviews: classified.map(c => ({
        conversation_id: c.conversation_id,
        snippet: (c.conversation_text || '').substring(0, 500),
        matched_keywords: c.matched_keywords || [],
        llm_result: {
          [matchField]: c.llm[matchField] ?? c.llm.is_match,
          category: c.llm.category,
    ...
    const reviewDir = join(reviewPath, '..');
    if (!fs.existsSync(reviewDir)) fs.mkdirSync(reviewDir, { recursive: true });
    fs.writeFileSync(reviewPath, JSON.stringify(reviewData, null, 2), 'utf8');
    console.log(`\nReview file written to ${reviewPath}`);
    console.log('Edit the file: set "correct" to true/false, optionally set "override_category"/"override_match", mark "promote_to_example" to true.');
    console.log('Then run: node scripts/llm_classify.mjs --apply-review <review.json> --topic <topic.json>');
  }
}

// ── Apply review: promote examples and report accuracy ────────────────

async function applyReview(reviewPath, topicPath) {
  const review = JSON.parse(fs.readFileSync(reviewPath, 'utf8'));
  const topicConfig = JSON.parse(fs.readFileSync(topicPath, 'utf8'));
  const matchField = topicConfig.match_field || review.match_field || 'is_match';

  const reviewed = review.reviews.filter(r => r.correct !== null);
  const agreed = reviewed.filter(r => r.correct === true);
  const corrected = reviewed.filter(r => r.correct === false);
  const requestedPromotion = review.reviews.filter(r => r.promote_to_example === true);
  const skippedUnreviewed = requestedPromotion.filter(r => r.correct === null);
  const toPromote = requestedPromotion.filter(r => r.correct !== null);

  console.log(`\n=== Review Summary ===`);
  console.log(`Total classified: ${review.reviews.length}`);
  console.log(`Reviewed by human: ${reviewed.length}`);
  console.log(`Agreed (correct): ${agreed.length} (${reviewed.length > 0 ? Math.round(agreed.length / reviewed.length * 100) : 0}%)`);
  console.log(`Corrected: ${corrected.length}`);
  console.log(`Requested promotions: ${requestedPromotion.length}`);
  console.log(`Skipped promotions (unreviewed): ${skippedUnreviewed.length}`);
  console.log(`To promote as examples: ${toPromote.length}`);

  if (corrected.length > 0) {
    console.log(`\nCorrections:`);
    for (const c of corrected) {
      const original = c.llm_result?.category || 'unknown';
      const override = c.override_category || 'not specified';
      console.log(`  ${c.conversation_id}: ${original} → ${override}`);
    }
  }

  if (toPromote.length > 0) {
    if (!topicConfig.few_shot_examples) topicConfig.few_shot_examples = [];
    let promotedCount = 0;
    let skippedInvalidCorrection = 0;
    for (const p of toPromote) {
      const llmResult = p.llm_result || {};
      const originalMatch = llmResult[matchField] ?? llmResult.is_match ?? false;
      let classification;

      if (p.correct === false) {
        const hasOverrideCategory = typeof p.override_category === 'string' && p.override_category.trim().length > 0;
        const hasOverrideMatch = typeof p.override_match === 'boolean';
        if (!hasOverrideCategory && !hasOverrideMatch) {
          skippedInvalidCorrection++;
          continue;
        }

        const correctedCategory = hasOverrideCategory ? p.override_category.trim() : (llmResult.category || 'other');
        const correctedMatch = hasOverrideMatch
          ? p.override_match
          : correctedCategory.toLowerCase() !== 'other';
        classification = {
          ...llmResult,
          [matchField]: correctedMatch,
          is_match: correctedMatch,
        ...
        classification = {
          ...llmResult,
          [matchField]: Boolean(originalMatch),
          is_match: Boolean(originalMatch),
        };
      }

      topicConfig.few_shot_examples.push({
        conversation_snippet: p.snippet,
        classification,
      });
      promotedCount++;
    }
    fs.writeFileSync(topicPath, JSON.stringify(topicConfig, null, 2), 'utf8');
    if (skippedInvalidCorrection > 0) {
      console.log(`Skipped ${skippedInvalidCorrection} corrected promotions missing override_category/override_match.`);
    }
    console.log(`\nPromoted ${promotedCount} examples to ${topicPath}`);
    console.log(`Total few-shot examples now: ${topicConfig.few_shot_examples.length}`);
  }
}

// ── Entry point ──────────────────────────────────────────────────────────

if (process.argv.includes('--apply-review')) {
  const args = process.argv.slice(2);
  let reviewPath = null;
  let topicPath = null;
  for (let i = 0; i < args.length; i++) {
    if (args[i] === '--apply-review' && args[i + 1]) reviewPath = args[++i];
    else if ((args[i] === '--topic' || args[i] === '--keywords') && args[i + 1]) topicPath = args[++i];
  }
  if (!reviewPath || !topicPath) {
    console.error('Usage: node scripts/llm_classify.mjs --apply-review <review.json> --topic <topic.json>');
    process.exit(1);
  }
  const resolvedReview = resolvePath(reviewPath) || reviewPath;
  const resolvedTopic = resolvePath(topicPath) || topicPath;
  applyReview(resolvedReview, resolvedTopic).catch(err => { console.error('Fatal:', err); process.exit(1); });
} else {
  main().catch(err => {
