...
**Performance:**
- **`--minimal` (fastest):** Skips contact filter and ICP coverage. ~5 min for 4 months.
- **Default:** Uses `tiene_cuenta_contador` field only. ~14 min for 4 months.
- **`--dual-criteria`:** Also checks association type 8 per deal. ~5-10 min for 4 months.

**Usage:**
```bash
# Minimal mode (fastest, ~5 min for 4 months)
python tools/scripts/hubspot/analyze_smb_accountant_involved_funnel.py --start-date 2025-10-01 --end-date 2026-02-01 --minimal
