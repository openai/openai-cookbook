...
```bash
# 1. Refresh Colppy first payments for a month
...
Run all months + export snapshot + publish plugin:
```bash
...
Both use **Argentina timezone** for timestamps.
...
2. **For NO_PRIMARY:** Add primary company association (type 5) with the company whose CUIT matches Colppy DB. Use `fix_deal_associations.py` or HubSpot UI.
3. **For MISMATCH:** Change primary company to the one with Colppy DB CUIT. Ensure that company has CUIT in HubSpot.
