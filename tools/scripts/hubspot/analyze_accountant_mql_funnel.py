...
def create_funnel_visualization(csv_file_path, output_dir=None):
    ...
    mql = int(row.get('MQL_Contador', 0))
    ...
    stages = ['MQL Contador', 'Deal Created', 'Deal Closed Won']
    ...
    ax1.set_title(f'Accountant MQL Funnel - {period}', fontsize=16, fontweight='bold', pad=20)
    ax1.axis('off')
    ...
    plt.suptitle('Accountant MQL Funnel Analysis Dashboard', fontsize=18, fontweight='bold', y=0.98)
...
def create_comparative_visualization(df, csv_file_path, output_dir=None):
    ...
    fig.suptitle('Accountant MQL Funnel - Multi-Month Comparison', fontsize=18, fontweight='bold', y=0.98)
    
    months = df['Month'].tolist()
    
    # Extract metrics
    mql_values = df['MQL_Contador'].tolist()
    ...
    ax1.bar([i - width for i in x], mql_values, width, label='MQL Contador', color='#4A90E2', alpha=0.8)
...
def main():
    ...
    parser.add_argument('--csv', type=str, help='Path to existing CSV file to generate visualization from (e.g., tools/outputs/accountant_mql_funnel_20251201_20260101.csv)')
