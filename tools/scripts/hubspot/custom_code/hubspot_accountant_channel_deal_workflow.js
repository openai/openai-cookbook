  ...
  async function getDealCollaborators(dealId, collaboratorIdsString = null) {
   ...
   try {
    ...
    try {
      ...
      console.log(`🔍 REQUESTED PROPERTIES: dealname, hubspot_owner_id, accountant_channel_involucrado_en_la_venta, amount, dealstage, hs_all_collaborator_owner_ids, primary_company_type`);
      ...
      deal = await client.crm.deals.basicApi.getById(dealId, [
        ...
        'primary_company_type'  // Synchronized property from Primary Company type
      ]);
    ...
    const primaryCompanyType = deal.properties.primary_company_type;  // Synchronized from Primary Company
    
    console.log(`Deal Name: ${dealName}`);
    console.log(`Deal Owner ID: ${dealOwnerId}`);
    console.log(`Current Accountant Channel Value: ${currentAccountantChannelValue}`);
    console.log(`Primary Company Type (synchronized): ${primaryCompanyType || 'Not set'}`);
    console.log(`Deal Amount: ${dealAmount}`);
    console.log(`Deal Stage: ${dealStage}`);
    
    // Log ICP Operador classification based on primary_company_type (same logic as analyze_icp_operador_billing.py)
    const ACCOUNTANT_COMPANY_TYPES = ['Cuenta Contador', 'Cuenta Contador y Reseller', 'Contador Robado'];
    if (primaryCompanyType && ACCOUNTANT_COMPANY_TYPES.includes(primaryCompanyType)) {
      console.log(`🎯 ICP OPERADOR DETECTED: Primary Company type "${primaryCompanyType}" indicates this deal is billed to accountant`);
      console.log(`   This matches the logic in analyze_icp_operador_billing.py`);
    } else if (primaryCompanyType) {
      console.log(`📊 ICP PYME: Primary Company type "${primaryCompanyType}" indicates this deal is billed to SMB`);
    } else {
      console.log(`⚠️ NO PRIMARY COMPANY TYPE: primary_company_type is not set - deal may not have Primary Company or sync may not be configured`);
    }
