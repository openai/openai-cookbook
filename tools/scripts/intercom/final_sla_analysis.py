#!/usr/bin/env python3
"""
Final SLA Compliance Analysis for Intercom Conversations
With corrected label extraction and actual labels found
"""
...
class FinalSLAAnalyzer:
    ...
    async def analyze_friday_conversations(self, target_date: str = "2025-08-29"):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            # Now get detailed information for closed conversations (limit to first 30 for analysis)
            closed_conversations = [c for c in all_conversations if c.get('state') == 'closed'][:30]
    ...
    def print_sla_results(self, sla_analysis: List[Dict]):
        ...
        print("-" * 70)
        print(f"{'Label':<35} {'Total':<8} {'Compliant':<10} {'Compliance %':<12} {'Avg Resolution':<15}")
        print("-" * 70)
        ...
        for label, stats in label_summary.items():
            ...
            print(f"{label:<35} {stats['total']:<8} {stats['compliant']:<10} {compliance_pct:<12.1f}% {avg_resolution:<15.1f}h")
        
        # Show detailed examples with SLA calculations
        print(f"\n📝 DETAILED EXAMPLES WITH SLA CALCULATIONS:")
        print("-" * 80)
        
        for i, analysis in enumerate(sla_analysis[:15]):  # Show first 15 examples
...
async def main():
    ...
    print("🚀 Final SLA Compliance Analysis for Friday Conversations")
    print("=" * 60)
    
    analyzer = FinalSLAAnalyzer(INTERCOM_ACCESS_TOKEN)
    ...
    if sla_analysis:
        ...
        # Save results
        output_file = f"/Users/virulana/openai-cookbook/tools/outputs/final_sla_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
