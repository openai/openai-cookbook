#!/usr/bin/env python3
"""
🔍 SINGLE MIXPANEL API TEST
Makes exactly ONE API call with complete logging

Usage: python3 single_test.py
"""
...
def make_single_api_call():
    """Make exactly ONE API call to Mixpanel"""
    
    log("🚀 Starting SINGLE API test")
    ...
    # Simple request for top 3 events
    endpoint = f"https://mixpanel.com/api/query/events/top?project_id={MIXPANEL_PROJECT_ID}&type=general&limit=3"
            ...
            try:
                ...
                if "error" in response_data:
                    ...
                    if "rate limit" in response_data['error'].lower():
                        log("🚫 CONFIRMED: Rate limit exceeded")
...
def main():
    log("=" * 60)
    log("🔍 SINGLE MIXPANEL API CALL TEST")
    log("=" * 60)
    
    # Check credentials
    if not all([MIXPANEL_PROJECT_ID, MIXPANEL_USERNAME, MIXPANEL_PASSWORD]):
        log("❌ Missing credentials in .env file")
        sys.exit(1)
    
    # Make exactly one API call
    success = make_single_api_call()
    
    log("=" * 60)
    if success:
        log("✅ RESULT: API call successful - No rate limit!")
        log("🎯 The API is working fine")
    else:
        log("❌ RESULT: API call failed")
        log("🎯 This confirms rate limiting or other issues")
    log("=" * 60)
