#!/usr/bin/env python3
"""
September 2025 Companies Analysis Script - Using HubSpot MCP Tools
...
def analyze_company_with_mcp(company_id: str, company_name: str) -> Dict:
    """Analyze a single company using MCP tools"""
    print(f"\n🔍 Analyzing: {company_name} (ID: {company_id})")
    
    try:
        # Import MCP tools
        from hubspot_custom_code_latest import get_hubspot_client
        
        client = get_hubspot_client()
        
        # Get company details
        company = client.crm.companies.basic_api.get_by_id(company_id)
        ...
        associations = client.crm.associations.batch_api.read(
        ...
        if primary_deals:
            deals = client.crm.deals.batch_api.read(
...
def main():
    ...
    for company_id, company_name in companies:
        result = analyze_company_with_mcp(company_id, company_name)
    ...
    if accountant_companies:
        ...
        for result in accountant_companies:
            status_emoji = "✅" if result.get('setup_correct', False) else "⚠️"
